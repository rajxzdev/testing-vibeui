  ...
  return (
    <div className={hero ? 'w-full max-w-2xl mx-auto' : 'w-full max-w-3xl mx-auto'}>
      ...
      <div className={'rounded-3xl border bg-white dark:bg-zinc-900/60 backdrop-blur transition ' +
        ...
        {/* Baris kontrol: di HP dibuat 2 baris supaya tombol tidak terdorong keluar layar */}
        <div className="px-3 pb-3 pt-1 space-y-2">
          <div className="flex items-center gap-1.5 min-w-0">
            <div className="flex items-center rounded-full border border-zinc-200 dark:border-zinc-800 p-0.5 shrink-0">
              {[
                ...
                <button key={m.k} onClick={() => onGenMode?.(m.k)} title={m.hint}
                  className={'rounded-full px-2.5 py-1 text-[11px] font-medium transition flex items-center gap-1 whitespace-nowrap ' +
                    (genMode === m.k
            ...
            <span className="text-[11px] text-zinc-400 dark:text-zinc-600 tabular-nums ml-auto shrink-0">{text.length}/300</span>
          </div>

          <div className="flex items-center justify-between gap-2">
            {typeof quotaLeft !== 'undefined'
              ? <span className="text-[11px] text-zinc-400 dark:text-zinc-600 truncate">{quotaLeft} kredit tersisa</span>
              : <span />}
            <button onClick={submit} disabled={!text.trim() || loading || disabled}
              className="shrink-0 rounded-full bg-zinc-900 dark:bg-white text-white dark:text-black h-9 px-4 text-sm font-medium flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition">
              {disabled ? <Lock className="h-4 w-4" /> : loading
                ? <span className="h-3.5 w-3.5 rounded-full border-2 border-current border-t-transparent animate-spin" />
                : <Bolt className="h-4 w-4" />}
              <span className="whitespace-nowrap">{loading ? 'Membuat…' : `Generate · ${genMode === 'live' ? 2 : 1}c`}</span>
            </button>
          </div>
        </div>
      </div>
